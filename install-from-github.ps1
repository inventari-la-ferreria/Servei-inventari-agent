...
function Install-Package {
    ...
    try {
        ...
        # URL directa de descarga de Google Drive (ID: 1mXGEbLdBuyeV4ejQ5U2iG2yFyxt1-oi7)
        $driveUrl = "https://drive.google.com/uc?export=download&id=1mXGEbLdBuyeV4ejQ5U2iG2yFyxt1-oi7"

        if (-not (Test-Path $credPath)) {
            try {
                Write-Host "Descargando credenciales desde Google Drive..." -ForegroundColor Cyan
                Invoke-WebRequest -Uri $driveUrl -OutFile $credPath -UseBasicParsing
                
                # Validar
                $json = Get-Content $credPath -Raw | ConvertFrom-Json
                if (-not $json.project_id) { throw "JSON inválido descargado de Drive" }
                
                Write-Host "✓ Credenciales descargadas e instaladas correctamente" -ForegroundColor Green
            }
            catch {
